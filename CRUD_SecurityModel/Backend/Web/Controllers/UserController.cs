...
namespace Web.Controllers
{
    ...
    {
        private readonly IBusiness<UserDTO, UserOptionsDTO> _userBusiness;
        private readonly UserBusiness _userBusinessPrueba;
        ...
        public UserController(IBusiness<UserDTO, UserOptionsDTO> userBusiness, UserBusiness userBusinessPrueba,ILogger<UserController> logger)
        {
            _userBusiness = userBusiness;
            _userBusinessPrueba = userBusinessPrueba;
            _logger = logger;
        }
        ...
        /// <summary>
        /// Obtiene todos los Users del sistema JWT
        /// </summary>
        /// <returns>Lista de Useres</returns>
        /// <response code="200">Retorna la lista de Users</response>
        /// <response code="500">Error interno del servidor</response>
        ...
        public async Task<IActionResult> GetAllUsersJWT()
        {
            try
            {
                ...
                var Users = await _userBusinessPrueba.GetAllAsyncJWT(includeInactive);
                return Ok(Users);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetUserById(int id)
        {
            ...
            {
                _logger.LogWarning(ex, "Validacion fallida para Obtener User con ID: {UserId}", id);
        ...
        [ProducesResponseType(400)]
        [ProducesResponseType(500)]
        public async Task<IActionResult> CreateUser([FromBody] UserOptionsDTO userOptionsDTO)
        {
            try
            {
                var createdUser = await _userBusiness.CreateAsync(userOptionsDTO);
        ...
        [ProducesResponseType(404)]
        [ProducesResponseType(500)]
        public async Task<IActionResult> UpdateUser([FromBody] UserOptionsDTO userOptionsDTO)
        {
            try
            {
                var updatedUser = await _userBusiness.UpdateAsync(userOptionsDTO);
                return Ok(updatedUser);
            }
            catch (ValidationException ex)
            {
                _logger.LogWarning(ex, "Validación fallida al actualizar el User con ID: {UserId}", userOptionsDTO.Id);
                return BadRequest(new { message = ex.Message });
            }
            catch (EntityNotFoundException ex)
            {
                _logger.LogInformation(ex, "No se encontró el user con ID: {UserId}", userOptionsDTO.Id);
                return NotFound(new { message = ex.Message });
            }
            catch (ExternalServiceException ex)
            {
                _logger.LogError(ex, "Error al actualizar el user con ID: {UserId}", userOptionsDTO.Id);
                return StatusCode(500, new { message = ex.Message });
            }
        }
