...
namespace Web.Controllers
{
    ...
    public class RolUserController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetRolUserById(int id)
        {
            ...
            {
                _logger.LogWarning(ex, "Validacion fallida para Obtener RolUser con ID: {RolUserId}", id);
        ...
        public async Task<IActionResult> UpdateRolUser([FromBody] RolUserOptionsDTO rolUserDTO)
        {
            ...
            {
                _logger.LogWarning(ex, "Validación fallida al actualizar el RolUser con ID: {RolUserId}", rolUserDTO.Id);
                return BadRequest(new { message = ex.Message });
            }
            catch (EntityNotFoundException ex)
            {
                _logger.LogInformation(ex, "No se encontró el RolUser con ID: {RolUserId}", rolUserDTO.Id);
                return NotFound(new { message = ex.Message });
            }
            catch (ExternalServiceException ex)
            {
