...
namespace Business
{
    ...
    ///</summary>
    public class UserBusiness : IBusiness<UserDTO, UserOptionsDTO>
    {
        private readonly IData<User> _userData;
        private readonly UserData _userDataPrueba;
        ...
        public UserBusiness(IData<User> userData, UserData userDataPrueba, IDeleteStrategyResolver<User> strategyResolver, ILogger<UserBusiness> logger,IMapper mapper)
        {
            _userData = userData;
            _userDataPrueba = userDataPrueba;
        ...
        public async Task<IEnumerable<UserDTO>> GetAllAsyncJWT(bool includeInactive = false)
        {
            try
            {
                var users = await _userDataPrueba.GetAllAsyncJWT(includeInactive);
                return _mapper.Map<IEnumerable<UserDTO>>(users);
            }
            catch (Exception ex)
            {
        ...
        /// </exception>
        public async Task<UserOptionsDTO> CreateAsync(UserOptionsDTO userOptionsDTO)
        {
            ValidateUser(userOptionsDTO);

            try
            {   
                userOptionsDTO.Status = true;

                var user = _mapper.Map<User>(userOptionsDTO);
                var createdUser = await _userData.CreateAsync(user);

                return _mapper.Map<UserOptionsDTO>(createdUser);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al crear un nuevo usuario: {Username}", userOptionsDTO?.Username ?? "null");
        ...
        /// </exception>
        public async Task<UserOptionsDTO> UpdateAsync(UserOptionsDTO userOptionsDTO)
        {
            if (userOptionsDTO.Id <= 0)
            {
                _logger.LogWarning("Intento de Actualizar un User con ID inválido: {UserId}", userOptionsDTO.Id);
                throw new ValidationException("id", "El ID del User debe ser mayor que cero.");
            }

            ValidateUser(userOptionsDTO);

            var existingUser = await _userData.GetByIdAsync(userOptionsDTO.Id);
            if (existingUser == null)
            {
                throw new EntityNotFoundException("User", userOptionsDTO.Id);
            }
            try
            {
                _mapper.Map(userOptionsDTO, existingUser);

                var updateUser = await _userData.UpdateAsync(existingUser);

                return _mapper.Map<UserOptionsDTO>(updateUser);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error al actualizar el usuario con ID: {UserId}", userOptionsDTO.Id);
        ...
        public async Task<bool> DeleteAsync(int id, DeleteType strategyType)
        {
            if (id <= 0)
            {
                _logger.LogWarning("Intento de Eliminar un User con ID inválido: {UserId}", id);
                throw new ValidationException("id", "El ID del User debe ser mayor que cero.");
            }
        ...
        /// </summary>
        private void ValidateUser(UserOptionsDTO userDTO)
        {
            if (userDTO == null)
            {
            ...
            {
                _logger.LogWarning("Intento de crear/actualizar un User con PersonId inválido.");
                throw new ValidationException("UserId", "El ID del Person debe ser mayor que cero.");
            }
        }
    }
}
