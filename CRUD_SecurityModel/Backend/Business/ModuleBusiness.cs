...
namespace Business
{
    ...
    public class ModuleBusiness : IBusiness<ModuleDTO, ModuleDTO>
    {
        ...
        public async Task<ModuleDTO> UpdateAsync(ModuleDTO moduleDTO)
        {
            if (moduleDTO.Id <= 0)
            {
                _logger.LogWarning("Se intentó Actualizar un Module con ID inválido: {ModuleId}", moduleDTO.Id);
                throw new Utilities.Exceptions.ValidationException("id", "El ID del Module debe ser mayor que cero");
            }
            ...
            try
            {
                ...
                var updateModule = await _moduleData.UpdateAsync(existingModule);

                return _mapper.Map<ModuleDTO>(updateModule);
            }
            catch (Exception ex)
            {
