...
namespace Business
{
    ...
    public class RolUserBusiness : IBusiness<RolUserDTO, RolUserOptionsDTO>
    {
        ...
        /// </exception>
        public async Task<RolUserOptionsDTO> UpdateAsync(RolUserOptionsDTO rolUserDTO)
        {
            if (rolUserDTO.Id <= 0)
            {
                _logger.LogWarning("Intento de obtener un RolUser con ID inválido: {RolUserId}", rolUserDTO.Id);
                throw new ValidationException("id", "El ID de RolUser debe ser mayor que cero.");
            }
            ...
            try
            {
                ...
                var updateRolUser = await _rolUserData.UpdateAsync(existingRolUser);

                return _mapper.Map<RolUserOptionsDTO>(updateRolUser);
            }
            catch (Exception ex)
            {
