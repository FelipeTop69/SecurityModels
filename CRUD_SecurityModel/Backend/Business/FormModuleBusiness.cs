...
namespace Business
{
    ...
    public class FormModuleBusiness : IBusiness<FormModuleDTO, FormModuleOptionsDTO>
    {
        ...
        /// </exception>
        public async Task<FormModuleOptionsDTO> UpdateAsync(FormModuleOptionsDTO formModuleDTO)
        {
            if (formModuleDTO.Id <= 0)
            {
                _logger.LogWarning("Intento de actualizar un FormModule con ID inválido: {FormModuleId}", formModuleDTO.Id);
                throw new ValidationException("id", "El ID de la relación FormModule debe ser mayor que cero.");
            }
            ...
            try
            {
                ...
                var updateFormModule = await _formModuleData.UpdateAsync(existingFormModule);

                return _mapper.Map<FormModuleOptionsDTO>(updateFormModule);
        ...
        public async Task<bool> DeleteAsync(int id, DeleteType strategyType)
        {
            if (id <= 0)
            {
                _logger.LogWarning("Intento de Eliminar un FormModule con ID inválido: {FormModuleId}", id);
                throw new ValidationException("id", "El ID de la relación FormModule debe ser mayor que cero.");
            }
