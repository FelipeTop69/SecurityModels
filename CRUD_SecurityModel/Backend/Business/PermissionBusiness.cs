...
namespace Business
{
    ...
    public class PermissionBusiness : IBusiness<PermissionDTO, PermissionDTO>
    {
        ...
        public async Task<PermissionDTO> UpdateAsync(PermissionDTO permissionDTO)
        {
            ...
            {
                _logger.LogWarning("Intento de Actualizar un Permission con ID inválido: {PermissionId}", permissionDTO.Id);
                throw new ValidationException("id", "El ID del Permission debe ser mayor que cero.");
            }
            ...
            try
            {
                ...
                var updatPermission = await _permissionData.UpdateAsync(existingPermission);

                return _mapper.Map<PermissionDTO>(updatPermission);
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> DeleteAsync(int id, DeleteType strategyType)
        {
            if (id <= 0)
            {
                _logger.LogWarning("Intento de Eliminar un Permission con ID inválido: {PermissionId}", id);
                throw new ValidationException("id", "El ID del Permission debe ser mayor que cero.");
            }
