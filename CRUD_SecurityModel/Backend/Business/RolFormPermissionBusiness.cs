...
namespace Business
{
    ...
    public class RolFormPermissionBusiness : IBusiness<RolFormPermissionDTO, RolFormPermissionOptionsDTO>
    {
        ...
        public async Task<RolFormPermissionOptionsDTO> CreateAsync(RolFormPermissionOptionsDTO rolFormPermissionDTO)
        {
            ...
            try
            {
                rolFormPermissionDTO.Status = true;
        ...
        /// </exception>
        public async Task<RolFormPermissionOptionsDTO> UpdateAsync(RolFormPermissionOptionsDTO rolFormPermissionDTO)
        {
            if (rolFormPermissionDTO.Id <= 0)
            {
                _logger.LogWarning("Intento de Actualizar un RolFormPermission con ID inválido: {RolFormPermissionId}", rolFormPermissionDTO.Id);
                throw new ValidationException("id", "El ID de RolFormPermission debe ser mayor que cero.");
            }
            ...
            try
            {
                _mapper.Map(rolFormPermissionDTO, existingRolFormPermission);

                var updateRolFormPermission = await _rolFormPermissionData.UpdateAsync(existingRolFormPermission);

                return _mapper.Map<RolFormPermissionOptionsDTO>(updateRolFormPermission);
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> DeleteAsync(int id, DeleteType strategyType)
        {
            if (id <= 0)
            {
                _logger.LogWarning("Intento de Eliminar un RolFormPermission con ID inválido: {RolFormPermissionId}", id);
                throw new ValidationException("id", "El ID de RolFormPermission debe ser mayor que cero.");
            }
