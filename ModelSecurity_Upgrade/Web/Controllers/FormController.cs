...
namespace Web.Controllers
{
    ...
    public class FormController : ControllerBase
    {
        ...
        /// <response code="404">Form no encontrado</response>
        /// <response code="500">Error interno del servidor</response>
        [HttpDelete("DeleteStrategy/{id}/")]
        [ProducesResponseType(200)]
        [ProducesResponseType(400)]
        [ProducesResponseType(404)]
        [ProducesResponseType(500)]
        public async Task<IActionResult> DeleteFormWithStrategy(int id, [FromQuery] DeleteType type = DeleteType.Permanent)
        {
            try
            {
                await _formBusiness.DeleteFormWithStrategyAsync(id, type);
                return Ok(new { message = $"Eliminación {type} con estrategia exitosa." });
            }
