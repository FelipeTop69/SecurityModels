...
namespace Business
{
    ...
    public class FormBusiness
    {
        ...
        /// </exception>
        public async Task<bool> DeleteFormWithStrategyAsync(int id, DeleteType deleteType)
        {
            if (id <= 0)
            {
            ...
            IDeleteStrategy strategy = deleteType switch
            {
                DeleteType.Logical => new LogicalDeleteStrategy(_formData),
                DeleteType.Permanent => new PermanentDeleteStrategy(_formData),
                _ => throw new ArgumentOutOfRangeException(nameof(deleteType), "Tipo de eliminación no soportado.")
            };

            var context = new DeleteContext(strategy);

            try
            {
                return await context.ExecuteStrategyAsync(id);
            }
            catch (Exception ex)
            {
